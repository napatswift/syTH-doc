...
def get_token_text(token: str):
    ...
    if token == "phonenumber_th":
        return "".join(
            [arabic2th(n) if n.isdigit() else n for n in faker.phone_number()]
        )
    if token == "month":
...
def get_doc_md(template):
    ...
    Args:
        template (str): A string containing markdown text with tokens to be
            replaced.

    Returns:
        str: A string containing the updated markdown text
            with tokens replaced by their corresponding values.
    """
...
def parse_string(value_str):
    """
    Parse a string and attempt to convert it
    to a boolean, integer, or float value.
    ...
    Returns:
        bool, int, float, or str: The parsed value.
            ...
            integer, or float, the original string is returned.
